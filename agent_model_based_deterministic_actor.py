...
class agent():
    ...
    def act(self, observation_direct, observation_indirect):
        ...
        with torch.no_grad():
            ...
            action_temp = self.policy_net_action_copy(state_latent)
    ...
    def learn_env(self, state_latent, action, state_next_latent, reward, survive):
        ...
        state_next_loss = torch.nn.functional.mse_loss(state_next_latent_lastframe_prediction,
                                                       state_next_latent_lastframe)
        ...
        # update target models
        # self.polyak_averaging(self.env_net, self.env_net_slow_copy, 0.001)
        ...
        for i in range(self.value_hallu_loops):
            ...
            with torch.no_grad():
                if i == 0:
                    ...
                    state_latent_hallu = state_next_latent_hallu
                    action_hallu = action_next_hallu
            ...
            value_next_hallu = self.value_net(state_next_latent_hallu, action_next_hallu)
            #working on this
    ...
    def build_env_network(self):
        ...
        class Env_Net(torch.nn.Module):
            ...
            def model_forward(self, state, action):
                ...
                y = self.model_net(c)
                state_next_lastframe = y.unsqueeze(1)# + self.state_extract_lastframe(state)
        ...
        self.env_net = Env_Net(self.model_net_structure, self.reward_net_structure, self.survive_net_structure).to(
            self.train_device)
