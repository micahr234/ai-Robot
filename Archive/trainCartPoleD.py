...
# Run simulation
env = gym.make('CartPoleBulletEnv-v1', renders=render, discrete_actions=True)
#env = gym.make('CartPole-v1')

num_of_action_values = [2, 2]
state_space_min = [-1, -1, -1, -1, 0]
state_space_max = [1, 1, 1, 1, 200]
reward_space_min = [0]
reward_space_max = [1]
agent = AgentTorchDiscrete("Cart2D", num_of_action_values, state_space_min, state_space_max, reward_space_min, reward_space_max,
                           discount=0.999, batch_size=1000, value_learn_rate=0.001, policy_learn_rate=0.0001, policy_copy_rate=0.1,
                           learn_iterations=100, memory_buffer_size=1000000, next_learn_factor=0.5)
...
for n in range(learn_episodes + test_episodes):
    ...
    while True:
        ...
        action_mod = action[0]  # debug only
        next_observation_partial, reward, done, info = env.step(action_mod)
        next_observation = np.concatenate((next_observation_partial, np.array(timestep, ndmin=1)))
        if n < learn_episodes:
