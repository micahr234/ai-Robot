...
class Memory():
    ...
    def __init__(self, capacity, filename):
        ...
        if self.filename.is_file():
            ...
            self.memory_terminate[0:index, :] = buffer['terminate'][0:index, :]
        else:
    ...
    def create(self, state_shape, action_shape):
        ...
        self.memory_terminate = torch.empty([self.capacity, 1], device='cpu', requires_grad=False)
    ...
    def add(self, state, action, reward, next_state, terminate):
        ...
        self.memory_terminate[index, :] = terminate
    ...
    def save(self):
        ...
        state, action, reward, next_state, terminate = self.get(index)
        torch.save({'state': state, 'action': action, 'reward': reward, 'next_state': next_state, 'terminate': terminate, 'length': self.length, 'position': self.position}, self.filename)
        pass