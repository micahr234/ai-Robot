...
unity_env = UnityEnvironment('environments/RoboPoint/RoboPoint.exe', base_port=10000, worker_id=np.random.randint(1000), no_graphics=False)
gym_env = UnityToGymWrapper(unity_env, False, False, True)

execute.execute(
    instance_name='RoboPoint1',

    environment_name='RoboPoint',
    environment=gym_env,
    ...
    action_random_prob=lambda i: 0.3 * (i < 900000),
    ...
    value_hallu_loops=1,
    value_polyak=0.0025,
    policy_learn_rate=lambda i: 0.00001,
